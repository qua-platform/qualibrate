...
test("Workflow1 - Running a Calibration Node", async ({ page }, testInfo) => {
  ...
  try {
    await expect(page.getByTestId("project-wrapper-init_project")).toBeVisible({ timeout: 10000 });  // This wait is too long, it is better to check if nodes-and-job-wrapper is visible
