...
def test_snapshot_get_snapshot_default(client_custom_settings, default_local_storage_project, snapshots_history):
    snapshot_id = 4
...
def test_snapshots_compare_by_id(client_custom_settings, snapshots_history):
    response = client_custom_settings.get("/api/snapshot/3/compare", params={"id_to_compare": 5})
    assert response.status_code == 200
    assert response.json() == {"/quam/node": {"old": 3, "new": 5}}

    response = client_custom_settings.get("/api/snapshot/5/compare", params={"id_to_compare": 3})
    assert response.status_code == 200
...
def test_snapshots_search_data_wildcard(client_custom_settings, s_id, data_path, result):
    response = client_custom_settings.get(
        f"/api/snapshot/{s_id}/search/data/values",
        params={"data_path": data_path},
    )
...
def test_snapshots_search_recursive_data(client_custom_settings, s_id, target_key, result):
    response = client_custom_settings.get(
...
def test_snapshot_update_entries(client_custom_settings, default_local_storage_project, mocker, tmp_path):
    quam_state_data = {
        "octaves": {
            "oct1": {
                "name": "oct1",
    ...
    response = client_custom_settings.post(
        ...
        json={"items": [{"data_path": f"#{item[0]}", "value": item[2]} for item in updates]},
    )
    assert response.status_code == 200
    new_snapshot_data = json.loads(snapshot_file.read_text())
    for update in updates:
        new_specific_data = json.loads(update[3].read_text())
        assert jsonpointer.resolve_pointer(new_specific_data, update[0]) == update[2]
        assert jsonpointer.resolve_pointer(new_snapshot_data, update[0]) == update[2]
