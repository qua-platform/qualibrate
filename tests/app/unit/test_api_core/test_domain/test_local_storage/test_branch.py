...
class TestLocalStorageBranch:
    ...
    def test_search_snapshots_data_ascending(self, mocker, settings, descending):
        suited_snapshot_ids = list(range(4, 0, -1) if descending else range(1, 5))
        patched_find_ids = mocker.patch(
            ("qualibrate.app.api.core.domain.local_storage.branch.find_nodes_ids_by_filter"),
            return_value=suited_snapshot_ids,
        )
        patched_snapshot = mocker.patch(
            ("qualibrate.app.api.core.domain.local_storage.branch.SnapshotLocalStorage"),
            side_effect=lambda _id, settings: _id,
        )
        patched_get_slice = mocker.patch(
            ("qualibrate.app.api.core.domain.local_storage.branch.get_page_slice"),
            side_effect=lambda snapshots, page_filter: list(snapshots),
        )
        patched_find_asc = mocker.patch(
            ("qualibrate.app.api.core.utils.find_utils.search_snapshots_data_with_filter_ascending"),
            side_effect=lambda snapshots, *args: snapshots,
        )
        patched_find_desc = mocker.patch(
            ("qualibrate.app.api.core.utils.find_utils.search_snapshots_data_with_filter_descending"),
            side_effect=lambda snapshots, *args: snapshots,
        )
        ...
        patched_snapshot.assert_has_calls([mocker.call(_id, settings=settings) for _id in suited_snapshot_ids])
        if descending:
            patched_find_asc.assert_not_called()
            patched_find_desc.assert_called_once_with(mocker.ANY, data_path, filter_no_change)
        else:
            patched_find_asc.assert_called_once_with(mocker.ANY, data_path, filter_no_change)
