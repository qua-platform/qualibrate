...
def test_read_minified_node_content_node_info_filled(mocker, settings):
    ...
    patched_get_id_local_path = mocker.patch(
        ("qualibrate.app.api.core.domain.local_storage.utils.snapshot_content.IdToLocalPath.get_path"),
        side_effect=[1, 2],
    )
    patched_is_file = mocker.patch("pathlib.Path.is_file")
    snapshot_path = NodePath(settings.storage.location.joinpath("2024-04-12", "#2_node_174011"))
...
def test_read_minified_node_content_node_info_empty_valid_id_file_exists(mocker, settings):
    ts = 1712932811
    ...
    patched_path_stat = mocker.patch("pathlib.Path.stat", return_value=FileStat())
    patched_path_parent = mocker.patch("pathlib.Path.parent", new_callable=PropertyMock)
    patched_is_file = mocker.patch("pathlib.Path.is_file", return_value=True)
    result = snapshot_content.read_minified_node_content({}, node_file, snapshot_path, settings)
    assert result == {
        "id": 2,
        "parents": [1],
    ...
    patched_get_id_local_path.assert_called_once_with("project", 1, settings.storage.location)
    patched_path_stat.assert_called_once()
    patched_path_parent.assert_not_called()


def test_read_minified_node_content_node_info_empty_no_id_no_file(mocker, settings):
    ts = 1712932811

    class FileStat:
        st_mtime = ts

    snapshot_path = NodePath(settings.storage.location.joinpath("2024-04-12", "node_dir"))
    snapshot_path.mkdir(exist_ok=True, parents=True)
    node_file = snapshot_path / "node_file.json"
    patched_get_id_local_path = mocker.patch(
        ("qualibrate.app.api.core.domain.local_storage.utils.snapshot_content.IdToLocalPath.get_path"),
    )
    patched_file_stat_patch = mocker.patch("pathlib.Path.stat", return_value=FileStat())
...
def test_read_metadata_node_content_node_info_not_filled(mocker, settings):
    ...
    snapshot_path = NodePath(settings.storage.location.joinpath("subpath", "node_name"))
    result = snapshot_content.read_metadata_node_content(
...
def test_get_node_filepath(snapshot_path):
    assert snapshot_content.get_node_filepath(snapshot_path) == snapshot_path / "node.json"
...
def test_get_data_node_path_valid(snapshot_path):
    ...
    result = snapshot_content.get_data_node_path(node_info, snapshot_path / "node.json", snapshot_path)
...
def test_update_state_dir_warns_on_missing_keys(tmp_path, caplog):
    ...
    (tmp_path / "wiring.json").write_text(json.dumps({"wiring": {}, "network": {}}))
    ...
    assert "Not all root items of the new quam state have been saved" in caplog.text
...
def test_update_active_machine_path_file(mocker, tmp_path, settings):
    ...
    path.write_text("{}")
    mocker.patch(
        ("qualibrate.app.api.core.domain.local_storage.utils.snapshot_content.get_quam_state_path"),
        return_value=path,
    )
...
def test_load_snapshot_metadata_from_node_content(mocker, snapshot_path, settings):
    snapshot_info = {"id": 1}
    mocker.patch(
        ("qualibrate.app.api.core.domain.local_storage.utils.snapshot_content.read_metadata_node_content"),
...
def test_load_snapshot_data_without_refs_from_node_content(snapshot_path, settings):
    snapshot_info = {}
...
def test_load_snapshot_data_machine_no_machine_key(mocker):
    patched_get_data_node_path = mocker.patch(
        ("qualibrate.app.api.core.domain.local_storage.utils.snapshot_content.get_data_node_path"),
        return_value=None,
    )
    patched_read_quam_content = mocker.patch(
        ("qualibrate.app.api.core.domain.local_storage.utils.snapshot_content.read_quam_content"),
    )
    snapshot_info = {"data": {}}
    assert (
    ...
    patched_get_data_node_path.assert_called_once_with({}, "node_path", "snapshot_path")
...
def test_load_snapshot_data_machine_from_node_content(mocker, settings):
    ...
    snapshot_info = {"data": {}}
    snapshot_content.load_snapshot_data_machine_from_node_content({}, "node_path", "snapshot_path", None, snapshot_info)
    assert snapshot_info["data"]["quam"] == {"machine": "..."}
    patched_get_data_node_path.assert_called_once_with({}, "node_path", "snapshot_path")
    patched_read_quam_content.assert_called_once_with("quam_path")


def test_load_snapshot_data_results_from_node_content(mocker, snapshot_path, settings):
    fake_storage = mocker.MagicMock()
    fake_storage.data = {"some": "results"}
    dfs_patch = mocker.patch(
        ("qualibrate.app.api.core.domain.local_storage.utils.snapshot_content.DataFileStorage"),
    ...
    fake_storage.load_from_flag.assert_called_once_with(StorageLoadTypeFlag.DataFileWithoutRefs)


def test_load_snapshot_data_results_with_imgs_from_node_content(mocker, snapshot_path, settings):
    fake_storage = mocker.MagicMock()
    ...
    dfs_patch.assert_called_once_with(snapshot_path, settings)
    fake_storage.load_from_flag.assert_called_once_with(StorageLoadTypeFlag.DataFileWithImgs)


def test_default_snapshot_content_updater_patches_exists(mocker, snapshot_path, settings):
    node_file = snapshot_path / "node.json"
    quam_file = snapshot_path / "quam.json"
    quam_file.write_text("{}")
    node_file.write_text(json.dumps({"data": {"quam": "quam.json"}, "patches": [{"patch1": "info"}]}))
    mocker.patch("qualibrate.app.api.core.domain.local_storage.utils.snapshot_content.update_active_machine_path")
...
def test_default_snapshot_content_updater_adds_patch_key(mocker, snapshot_path, settings):
    node_file = snapshot_path / "node.json"
    quam_file = snapshot_path / "quam.json"
    quam_file.write_text("{}")
