...
node.machine = machine = BasicQuam(
    channels={
        ...
        "ch2": SingleChannel(opx_output=("con1", 2), intermediate_frequency=75e6),
        "ch3": SingleChannel(opx_output=("con1", 3), intermediate_frequency=125e6),
    }
)
...
if node.parameters and node.parameters.targets is not None:
    node.outcomes = {target: "successful" if bool(randint(0, 1)) else "failed" for target in node.parameters.targets}
