...
def validate_fields(original_instance, built_class):
    ...
    assert resonator_field.annotation == Parameters.model_fields["resonator"].annotation
    ...
    for field_name in original_instance.model_fields_set - {"resonator"}:
        assert original_instance.model_fields[field_name] == built_class.model_fields[field_name]
