...
class TestNodeSerialization:
    ...
    def test_serialize_with_machine_no_attributes(self, base_node):
        ...
        base_node.machine = Mock(spec=[])  # Machine without active_qubits or qubits
    ...
    def test_serialize_active_qubit_identification(self, base_node, mock_qubits):
    ...
    def test_serialize_qubit_without_averaged_fidelity(self, base_node):
        """Test serialization when gate_fidelity doesn't have 'averaged' attribute"""
        qubits = {
            'qA1': Mock(),
        }
        qubits['qA1'].name = 'qA1'
    ...
    def test_serialize_metadata_structure(self, node_with_machine):
        ...
        for qubit_name, qubit_data in metadata.items():
...
class TestNodeSerializationEdgeCases:
    ...
    def test_serialize_large_number_of_qubits(self):
        ...
        # Create 100 qubits
        qubits = {
            f'q{i}': MockQubit(f'q{i}', 0.95 + i * 0.0001)
            for i in range(100)
        }
